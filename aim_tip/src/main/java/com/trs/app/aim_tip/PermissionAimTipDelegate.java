 ...
 * 在申请权限的时候自动提示。
 */
public class PermissionAimTipDelegate implements ActivityCompat.PermissionCompatDelegate {
    static PermissionAimTipDelegate delegate;
    /**
    ...
    private PermissionAimTipDelegate(AimTipShowController showController) {
     ...
     */
    public static synchronized void initPermissionTip(AimTipShowController showController) {
        if (delegate != null) {
            return;
        }
        delegate = new PermissionAimTipDelegate(showController);
    ...
    private boolean checkNeedPermission(String string,PackageManager packageManager,String PackageName) {
        if (PackageManager.PERMISSION_GRANTED == packageManager.checkPermission(string,PackageName)){
            //已经授予就不需要重复获取
            return false;
        }
        return true;
